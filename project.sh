...
URL="$OPENSHIFT_API_URL/oapi/ --insecure"
LOGIN=`oc whoami`
PROJECT_NAME_JENKINS=$1
PROJECT_NAME_THUNDER=$2

RBAC_FILE="create_project.yaml"
...
rollout_func () {
    ...
    if [[ ${check_resource} ]]
        then
            ...
            if [[ ! "$( oc rollout status dc/mysql -n ${5} --watch=false | grep successfully )" ]]
                then
                    printf "${1}################${2}\n"
                    printf "${3}Wait for Rollout MYSQL !!!${2}\n"
                    response_api_check
                    oc rollout status dc/mysql -n ${5}
                    printf "${1}################${2}\n"
            fi
            if [[ "$( oc rollout status dc/mysql -n ${5} --watch=false | grep successfully )" ]]
                then
                    printf "${1}################${2}\n"
                    printf "${3}Adding permotion to join network !!!${2}\n"
                    response_api_check
...
process_func () {
        ...
        TIME=30
        response_api_check > /dev/null
        if [[ "$( oc process -f ${8} -p JENKINS_PROJECT_NAME=${5} -p THUNDER_PROJECT_NAME=${6} | oc apply --dry-run=true -f - | grep configured )" ]]
            then
                        printf "${1}################${2}\n"
                        printf "${3}This Resourses exists !!!${2}\n"
                        printf "${1}################${2}\n"
                        response_api_check
                        oc process -f ${8} -p JENKINS_PROJECT_NAME=${5} -p THUNDER_PROJECT_NAME=${6} | oc  apply --dry-run=true -f - | grep configured
                        printf "${1}################${2}\n"
                ...
                if [ "$9" == "apply" ]
                    then
                        response_api_check
                        oc process -f ${8} -p JENKINS_PROJECT_NAME=${5} -p THUNDER_PROJECT_NAME=${6} | oc  apply --dry-run=false -f -
                        # printf "${1}Sleep for ${TIME} sec befoure next step ${2}\n"
                        rollout_func ${1} ${2} ${3} ${5} ${6}
                        # sleep ${TIME}
                    else
...
if [ "$3" == "apply" ]
    then
        ...
        process_func ${RED} ${NC} ${LIGHT_GREAN} ${PROCESS} $JENKINS_PROJECT_NAME $THUNDER_PROJECT_NAME "Create Projects" $RBAC_FILE ${3}
...
printf "${LIGHT_GREAN}$OPENSHIFT_API_URL/console${NC}\n"
printf "${RED}################${NC}\n"
